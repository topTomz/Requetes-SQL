...
DECLARE @LST_SOCIETES TABLE(ID INT IDENTITY, NOM_SOCIETE VARCHAR(100))-- Table temporaire pour stocker le nom des societes à traiter
...
-- Cette partie Permet de preparer un paterne pour supprimer un ligne dans DATA_ADMIN_EMAIL_GRP_WEKAFR si toutes les entreprises son à NULL -->
...
BEGIN
	IF @Sql_maker_delete is NULL -- Cette partie permet d'eviter AND au debut de la requetes de suppression
		SET @Sql_maker_delete ='DELETE FROM DATA_ADMIN_EMAIL_GRP_WEKAFR WHERE '+(SELECT NOM_SOCIETE FROM @LST_SOCIETES WHERE ID = @counter)+' IS NULL' -- requette de suppression d'une ligne si toutes les societes sont à null
...
BEGIN
    ...
    WHERE ID = @counter),'[_]%');
    -- Incrementation du compteur pour passer à la societe suivante
    SET @counter = @counter + 1;
...
--DELETE FROM TMP_USERS_ONLY WHERE  PATINDEX('%[^a-z,0-9,@,.,_]%', REPLACE(MAIL, '-', 'a')) = 1
	...
	SET @sql= 'MERGE INTO DATA_ADMIN_EMAIL_GRP_WEKAFR T -- table à cible (target)
				...
				WHEN MATCHED THEN -- quand ça match
					UPDATE SET T.'+@societe_current+' = S.DERNIERE_ACTIVITE_DT , T.'+@societe_current+'_MAIL_CODE = S.MAIL_CODE -- il met à jour
